//#region D:/Codex-work/ahsr-20260920/node_modules/@mapbox/mapbox-gl-style-spec/dist/index.es.js
var v8 = {
	$version: 8,
	$root: {
		version: {
		...
		lights: {
			...
			"sdk-support": { "basic functionality": {
				js: "3.0.0",
				android: "11.0.0",
				ios: "11.0.0"
			} }
	...
	option: {
		...
		type: {
			...
			values: {
				string: { doc: "The result will be coerced to a string." },
				number: { doc: "The result will be coerced to a number." },
				boolean: { doc: "The result will be coerced to a boolean." },
				color: { doc: "The result will be coerced to a color." },
				object: {
	...
	properties_light_flat: {
		anchor: {
			...
			values: {
				...
				viewport: { doc: "The position of the light source is aligned to the rotation of the viewport. If terrain is enabled, performance regressions may occur in certain scenarios, particularly on lower-end hardware. Ensure that you test your target scenarios on the appropriate hardware to verify performance." }
			},
			"property-type": "data-constant",
			transition: false,
			expression: {
	...
	iconset: ["iconset_sprite", "iconset_source"],
	iconset_sprite: {
		type: {
			required: true,
			type: "enum",
			values: { sprite: { doc: "A sprite icon set." } },
	...
	iconset_source: {
		type: {
			required: true,
			type: "enum",
			values: { source: { doc: "A source icon set." } },
			doc: "The type of the icon set."
		},
		source: {
			required: true,
	...
	source_raster_array: {
		...
		type: {
			required: true,
			type: "enum",
			values: { "raster-array": { doc: "A raster array source" } },
			doc: "The type of the source."
		},
		url: {
			type: "string",
	...
	source_geojson: {
		...
		dynamic: {
			...
			"sdk-support": { "basic functionality": { js: "3.4.0" } }
		}
	},
	source_video: {
		type: {
	...
	source_model: {
		type: {
			...
			type: "enum",
			values: {
				model: { doc: "A collection of 3D models" },
				"batched-model": { doc: "A collection of 3D models with anchor data" }
			},
	...
	layout_model: {
		...
		"model-id": {
			...
			transition: false,
			requires: [{ source: ["geojson", "vector"] }]
		},
	...
	layout_clip: {
		"clip-layer-types": {
			...
			value: "enum",
			values: {
				model: { doc: "If present the clip layer would remove all 3d model layers below it. Currently only instanced models (e.g. trees) are removed." },
				symbol: { doc: "If present the clip layer would remove all symbol layers below it." }
			},
			"default": [],
	...
	layout_fill: {
		...
		"fill-elevation-reference": {
			...
			values: {
				none: { doc: "Elevated rendering is disabled." },
				"hd-road-base": { doc: "Elevate geometry relative to HD roads. Use this mode to describe base polygons of the road networks." },
				"hd-road-markup": { doc: "Elevated rendering is enabled. Use this mode to describe additive and stackable features such as 'hatched areas' that should exist only on top of road polygons." }
			},
			"default": "none",
			experimental: true,
			"private": true,
	...
	layout_circle: {
		...
		"circle-elevation-reference": {
			...
			values: {
				none: { doc: "Elevated rendering is disabled." },
				"hd-road-markup": { doc: "Elevated rendering is enabled. Use this mode to describe additive and stackable features that should exist only on top of road polygons." }
			},
			"default": "none",
			experimental: true,
			transition: false,
	...
	layout_building: {
		...
		"building-roof-shape": {
			type: "enum",
			values: {
				flat: { doc: "Use flat roof for the procedurally generated buildings." },
				hipped: { doc: "Use hipped roof for the procedurally generated buildings." },
				gabled: { doc: "Use gabled roof for the procedurally generated buildings." },
				parapet: { doc: "Use parapet roof for the procedurally generated buildings." },
				mansard: { doc: "Use mansard roof for the procedurally generated buildings." },
				skillion: { doc: "Use skillion roof for the procedurally generated buildings." },
				pyramidal: { doc: "Use pyramidal roof for the procedurally generated buildings." }
			},
			"default": "flat",
	...
	layout_line: {
		...
		"line-join": {
			...
			values: {
				...
				none: { doc: "Line segments are not joined together, each one creates a separate line. Useful in combination with line-pattern. Line-cap property is not respected. Can't be used with data-driven styling." }
			},
			"default": "miter",
		...
		"line-elevation-reference": {
			...
			values: {
				none: { doc: "Elevated rendering is disabled." },
				sea: { doc: "Elevated rendering is enabled. Use this mode to elevate lines relative to the sea level." },
				ground: { doc: "Elevated rendering is enabled. Use this mode to elevate lines relative to the ground's height below them." },
				"hd-road-markup": { doc: "Elevated rendering is enabled. Use this mode to describe additive and stackable features that should exist only on top of road polygons." }
			},
			"default": "none",
			transition: false,
		...
		"line-width-unit": {
			...
			values: {
				pixels: { doc: "Width is rendered in pixels." },
				meters: { doc: "Width is rendered in meters." }
			},
			"default": "pixels",
			experimental: true,
			"private": true,
	...
	layout_symbol: {
		...
		"symbol-z-order": {
			type: "enum",
			values: {
				auto: { doc: "Sorts symbols by `symbol-sort-key` if set. Otherwise, sorts symbols by their y-position relative to the viewport if `icon-allow-overlap` or `text-allow-overlap` is set to `true` or `icon-ignore-placement` or `text-ignore-placement` is `false`." },
				"viewport-y": { doc: "Sorts symbols by their y-position relative to the viewport if any of the following is set to `true`: `icon-allow-overlap`, `text-allow-overlap`, `icon-ignore-placement`, `text-ignore-placement`." },
				source: { doc: "Sorts symbols by `symbol-sort-key` if set. Otherwise, no sorting is applied; symbols are rendered in the same order as the source data." }
			},
			"default": "auto",
		...
		"symbol-z-elevate": {
			...
			requires: [{ "symbol-placement": ["point"] }, { "symbol-z-order": ["auto"] }],
			expression: {
				interpolated: false,
				parameters: ["zoom"]
			},
		...
		"symbol-elevation-reference": {
			...
			values: {
				sea: { doc: "Elevate symbols relative to the sea level." },
				ground: { doc: "Elevate symbols relative to the ground's height below them." },
				"hd-road-markup": { doc: "Use this mode to enable elevated behavior for features that are rendered on top of 3D road polygons. The feature is currently being developed." }
			},
			"default": "ground",
			experimental: true,
			"sdk-support": { "basic functionality": {
			...
			requires: [{ "symbol-z-elevate": true }],
			expression: {
				interpolated: false,
				parameters: ["zoom"]
			},
		...
		"icon-size-scale-range": {
			...
			experimental: true,
			expression: { interpolated: false },
			"sdk-support": { "basic functionality": { js: "3.8.0" } },
			"property-type": "data-constant"
		},
		"icon-text-fit": {
			type: "enum",
		...
		"text-writing-mode": {
			...
			value: "enum",
			values: {
				horizontal: { doc: "If a text's language supports horizontal writing mode, symbols would be laid out horizontally." },
				vertical: { doc: "If a text's language supports vertical writing mode, symbols would be laid out vertically." }
			},
	...
	"layout_raster-particle": { visibility: {
		type: "enum",
		values: {
			visible: { doc: "The layer is shown." },
			none: { doc: "The layer is not shown." }
	...
	camera: { "camera-projection": {
		...
		type: "enum",
		values: {
			perspective: { doc: "linear projection where distant objects appear smaller than closer objects. Lines that are parallel seem to converge towards a vanishing point" },
			orthographic: { doc: "Projection where objects are of the same scale regardless of whether they are far away or near to the camera. Parallel lines remains parallel and there is no vanishing point." }
		},
		"sdk-support": { "basic functionality": {
			js: "3.0.0",
			android: "11.0.0",
	...
	projection: {
		name: {
			type: "enum",
			values: {
				albers: { doc: "An Albers equal-area projection centered on the continental United States. You can configure the projection for a different region by setting `center` and `parallels` properties. You may want to set max bounds to constrain the map to the relevant region." },
				equalEarth: { doc: "An Equal Earth projection." },
				equirectangular: { doc: "An Equirectangular projection. This projection is very similar to the Plate Carrée projection." },
				lambertConformalConic: { doc: "A Lambert conformal conic projection. You can configure the projection for a region by setting `center` and `parallels` properties. You may want to set max bounds to constrain the map to the relevant region." },
				mercator: { doc: "The Mercator projection is the default projection." },
				naturalEarth: { doc: "A Natural Earth projection." },
				winkelTripel: { doc: "A Winkel Tripel projection." },
				globe: { doc: "A globe projection." }
			},
			"default": "mercator",
		...
		center: {
			...
			"sdk-support": { "basic functionality": { js: "2.6.0" } }
		},
		parallels: {
			type: "array",
			length: 2,
	...
	paint_fill: {
		...
		"fill-z-offset": {
			...
			experimental: true,
			"sdk-support": {
				"basic functionality": { js: "3.7.0" },
				"data-driven styling": { js: "3.7.0" }
			},
			expression: {
				interpolated: true,
				parameters: ["zoom", "feature"]
	...
	"paint_fill-extrusion": {
		...
		"fill-extrusion-height-alignment": {
			...
			experimental: true,
			values: {
				terrain: { doc: "The fill extrusion height follows terrain slope." },
				flat: { doc: "The fill extrusion height is flat over terrain." }
			},
		...
		"fill-extrusion-base-alignment": {
			...
			experimental: true,
			values: {
				terrain: { doc: "The fill extrusion base follows terrain slope." },
				flat: { doc: "The fill extrusion base is flat over terrain." }
			},
			doc: "Controls the behavior of fill extrusion base over terrain",
			"default": "terrain",
			requires: ["fill-extrusion-base"],
			"sdk-support": { "basic functionality": {
				js: "3.8.0",
				android: "11.8.0",
				ios: "11.8.0"
		...
		"fill-extrusion-ambient-occlusion-wall-radius": {
			...
			requires: ["lights", "fill-extrusion-edge-radius"],
			"sdk-support": { "basic functionality": {
				js: "3.0.0",
				android: "11.0.0",
				ios: "11.0.0"
	...
	paint_line: {
		...
		"line-blend-mode": {
			...
			experimental: true,
			"private": true,
			"sdk-support": { "basic functionality": { js: "3.21.0" } },
			expression: {
				interpolated: false,
				parameters: ["zoom"]
			},
	...
	paint_raster: {
		...
		"raster-color-scale": {
			...
			values: {
				linear: { doc: "Raster values are spaced evenly across the color ramp." },
				log: { doc: "Raster values are spaced logarithmically, giving more of the color ramp to smaller values. Useful for data concentrated near the low end of a wide range." }
			},
			"default": "linear",
			requires: ["raster-color", { source: "raster-array" }],
			expression: {
				interpolated: false,
				parameters: []
			},
		...
		"raster-elevation-reference": {
			...
			values: {
				sea: { doc: "Use this mode to elevate raster layers relative to the sea level." },
				ground: { doc: "Use this mode to elevate raster layers relative to the ground's height below them." }
			},
			"default": "sea",
			transition: false,
			experimental: true,
	...
	paint_background: {
		"background-pitch-alignment": {
			type: "enum",
			values: {
				map: { doc: "The background is aligned to the plane of the map." },
				viewport: { doc: "The background is aligned to the plane of the viewport, covering the whole screen. Note: This mode disables the automatic reordering of the layer when terrain or globe projection is used." }
			},
			"default": "map",
	...
	paint_sky: {
		"sky-type": {
			type: "enum",
			values: {
				gradient: { doc: "Renders the sky with a gradient that can be configured with `sky-gradient-radius` and `sky-gradient`." },
				atmosphere: { doc: "Renders the sky with a simulated atmospheric scattering algorithm, the sun direction can be attached to the light position or explicitly set through `sky-atmosphere-sun`." }
			},
			"default": "atmosphere",
	...
	paint_model: {
		...
		"model-type": {
			type: "enum",
			values: {
				"common-3d": { doc: "Integrated to 3D scene, using depth testing, along with terrain, fill-extrusions and custom layer." },
				"location-indicator": { doc: "Displayed over other 3D content, occluded by terrain." }
			},
			"sdk-support": { "basic functionality": {
				js: "3.0.0",
				android: "11.0.0",
		...
		"model-front-cutoff": {
			...
			doc: "An array for configuring the fade-out effect for the front cutoff of content on pitched map views. It contains three values: start, range and final opacity. The start parameter defines the point at which the fade-out effect begins, with smaller values causing the effect to start earlier. The range parameter specifies how long the fade-out effect will last. A value of 0.0 for range makes content disappear immediately without a fade-out effect. The final opacity determines content opacity at the end of the fade-out effect. A value of 1.0 for final opacity means that the cutoff is completely disabled. Note: The property has no effect on the map if terrain or globe projection is enabled.",
			"sdk-support": { "basic functionality": { js: "3.5.0" } }
		},
		"model-elevation-reference": {
			...
			values: {
				sea: { doc: "Elevated rendering is enabled. Use this mode to elevate models relative to the sea level." },
				ground: { doc: "Elevated rendering is enabled. Use this mode to elevate models relative to the ground's height below them." },
				"hd-road-markup": { doc: "Elevated rendering is enabled. Use this mode to describe additive and stackable features that should exist only on top of road polygons." }
			},
			"default": "ground",
			transition: false,
		...
		"model-line-cutout-mode": {
			...
			"default": "enabled",
			values: {
				enabled: { doc: "The model receives the cutout without any special handling." },
				disabled: { doc: "The cutout effect is disabled for the model." },
				"enabled-above-cutout": { doc: "Only enabled if the model is above the cutout plane, such as for models above cut tunnels." }
			},
			transition: false,
			expression: { interpolated: false },
			"property-type": "data-constant",
	...
	$doc: {
		...
		"use-theme": {
			...
			values: {
				"default": { doc: "The color theme is applied to this property (default behavior)." },
				none: { doc: "The color theme is ignored for this property." }
			},
			"default": "default"
		}
	}
...
function typeEquals(a, b) {
	if (a.kind === "array" && b.kind === "array") return a.N === b.N && typeEquals(a.itemType, b.itemType);
...
var Color = class Color {
	...
	*/
	static parse(input) {
		...
		if (!rgba) return;
		const color = new Color(rgba[0] / 255, rgba[1] / 255, rgba[2] / 255, rgba[3]);
...
var RenderColor = class {
	constructor(lut, r, g, b, a, premultiplied = false) {
		...
		if (!lut) {
			...
			this.r = lerp(lerp(lerp(data[i0], data[i1], bw), lerp(data[i2], data[i3], bw), gw), lerp(lerp(data[i4], data[i5], bw), lerp(data[i6], data[i7], bw), gw), rw) / 255 * (this.premultiplied ? a : 1);
			this.g = lerp(lerp(lerp(data[i0 + 1], data[i1 + 1], bw), lerp(data[i2 + 1], data[i3 + 1], bw), gw), lerp(lerp(data[i4 + 1], data[i5 + 1], bw), lerp(data[i6 + 1], data[i7 + 1], bw), gw), rw) / 255 * (this.premultiplied ? a : 1);
			this.b = lerp(lerp(lerp(data[i0 + 2], data[i1 + 2], bw), lerp(data[i2 + 2], data[i3 + 2], bw), gw), lerp(lerp(data[i4 + 2], data[i5 + 2], bw), lerp(data[i6 + 2], data[i7 + 2], bw), gw), rw) / 255 * (this.premultiplied ? a : 1);
			this.a = a;
		}
	}
	/**
	...
	toHslaArray() {
		...
		switch (max) {
			...
			case green:
				...
				break;
			default: h = ((red - green) / delta + 4) * 60;
		}
		return [
			h,
			s * 100,
...
var NonPremultipliedRenderColor = class extends RenderColor {
	constructor(lut, r, g, b, a) {
		super(lut, r, g, b, a, false);
	}
};
var PremultipliedRenderColor = class extends RenderColor {
	constructor(lut, r, g, b, a) {
...
var Formatted = class Formatted {
	...
	serialize() {
		...
		for (const section of this.sections) {
			...
			const options = {};
			if (section.fontStack) options["text-font"] = ["literal", section.fontStack.split(",")];
			if (section.scale) options["font-scale"] = section.scale;
			if (section.textColor) options["text-color"] = ["rgba"].concat(section.textColor.toNonPremultipliedRenderColor(null).toArray());
...
var ImageVariant = class ImageVariant {
...
var ResolvedImage = class ResolvedImage {
	...
	toString() {
		if (this.primaryId && this.secondaryId) return `[${this.primaryId.name},${this.secondaryId.name}]`;
...
function validateHSLA(h, s, l, a) {
	if (!(typeof h === "number" && h >= 0 && h <= 360)) return `Invalid hsla value [${(typeof a === "number" ? [
		h,
		s,
		l,
		a
	...
	if (!(typeof s === "number" && s >= 0 && s <= 100 && typeof l === "number" && l >= 0 && l <= 100)) return `Invalid hsla value [${(typeof a === "number" ? [
		h,
		s,
		l,
		a
	...
	if (!(typeof a === "undefined" || typeof a === "number" && a >= 0 && a <= 1)) return `Invalid hsla value [${[
		h,
		s,
		l,
		a
...
function toString(value) {
	...
	else if (typeof value === "string" || typeof value === "number" || typeof value === "boolean") return String(value);
	else if (value instanceof Formatted || value instanceof ResolvedImage || value instanceof Color) return value.toString();
...
var Literal = class Literal {
	...
	serialize() {
		if (this.type.kind === "array" || this.type.kind === "object") return ["literal", this.value];
		else if (this.value instanceof Color) return ["rgba"].concat(this.value.toNonPremultipliedRenderColor(null).toArray());
...
var Assertion = class Assertion {
	...
	static parse(args, context) {
		...
		if (name === "array") {
			...
			if (args.length > 2) {
				const type2 = args[1];
				if (typeof type2 !== "string" || !(type2 in types$1) || type2 === "object") return context.error("The item type argument of \"array\" must be one of string, number, boolean", 1);
	...
	evaluate(ctx) {
		for (let i = 0; i < this.args.length; i++) {
			...
			else if (i === this.args.length - 1) throw new RuntimeError(`The expression ${JSON.stringify(this.args[i].serialize())} evaluated to ${toString$1(typeOf(value))} but was expected to be of type ${toString$1(this.type)}.`);
		}
		return null;
	}
	eachChild(fn) {
...
var FormatExpression = class FormatExpression {
	...
	evaluate(ctx) {
		const evaluateSection = (section) => {
			...
			if (typeEquals(typeOf(evaluatedContent), ResolvedImageType)) return new FormattedSection("", evaluatedContent, null, null, null);
...
var ImageExpression = class ImageExpression {
	...
	static parse(args, context) {
		if (args.length < 2) return context.error(`Expected two or more arguments.`);
		let nextArgId = 1;
		...
		function tryParseOptions() {
			if (nextArgId < args.length) {
				...
				if (!isImageOptions(options)) return true;
		...
		for (let i = 0; i < 2; i++) if (!tryParseImage() || !tryParseOptions()) return;
	...
	evaluateParams(ctx, params) {
		const result = {};
		if (params) {
			for (const key in params) if (params[key]) try {
	...
	eachChild(fn) {
		...
		if (this.paramsPrimary) {
			for (const key in this.paramsPrimary) if (this.paramsPrimary[key]) fn(this.paramsPrimary[key]);
		}
		if (this.nameSecondary) {
			fn(this.nameSecondary);
			if (this.paramsSecondary) {
				for (const key in this.paramsSecondary) if (this.paramsSecondary[key]) fn(this.paramsSecondary[key]);
			}
		}
	}
	outputDefined() {
	...
	serializeOptions(params, iconsetId) {
		const result = {};
		if (iconsetId) result.iconset = { id: iconsetId };
		if (params) {
			result.params = {};
			for (const key in params) if (params[key]) result.params[key] = params[key].serialize();
		}
...
var Coercion = class Coercion {
	...
	static parse(args, context) {
		...
		if (name === "to-array") {
			...
			if (context.expectedType) {
				if (context.expectedType.kind === "array") type = array$1(context.expectedType.itemType, arrayLength);
				else return context.error(`Expected ${context.expectedType.kind} but found array.`);
			...
			for (let i = 0; i < arrayLength; i++) {
				...
				let parsedMember;
				if (Array.isArray(member)) parsedMember = context.parse(member, void 0, type.itemType);
				else {
					const memberType = getType(member);
					if (memberType !== type.itemType.kind) return context.error(`Expected ${type.itemType.kind} but found ${memberType}.`);
	...
	evaluate(ctx) {
		...
		} else if (this.type.kind === "formatted") return Formatted.fromString(toString(this.args[0].evaluate(ctx)));
		else if (this.type.kind === "resolvedImage") return ResolvedImage.build(toString(this.args[0].evaluate(ctx)));
		else if (this.type.kind === "array") return this.args.map((arg) => {
...
var CompoundExpression = class CompoundExpression {
	...
	static parse(args, context) {
		...
		for (const [params, evaluate] of availableOverloads) {
			...
			overloadIndex++;
			if (signatureContext === null) signatureContext = context._forkForSignature();
			else signatureContext.errors.length = 0;
			const parsedArgs = [];
			...
			if (signatureContext.errors.length === 0) return new CompoundExpression(op, type, evaluate, parsedArgs, overloadIndex);
		}
		if (overloadParams.length === 1) context.errors.push(...signatureContext.errors);
		else {
			const signatures = (overloadParams.length ? overloadParams : availableOverloads.map(([params]) => params)).map(stringifySignature).join(" | ");
			const actualTypes = [];
			for (let i = 1; i < args.length; i++) {
...
function pointWithinPolygons(point, polygons) {
	for (let i = 0; i < polygons.length; i++) if (pointWithinPolygon(point, polygons[i])) return true;
	return false;
}
function lineIntersectPolygon(p1, p2, polygon) {
	for (const ring of polygon) for (let j = 0, len = ring.length, k = len - 1; j < len; k = j++) {
		const q1 = ring[k];
		const q2 = ring[j];
		if (segmentIntersectSegment(p1, p2, q1, q2)) return true;
	}
	return false;
}
function lineStringWithinPolygon(line, polygon) {
	for (let i = 0; i < line.length; ++i) if (!pointWithinPolygon(line[i], polygon)) return false;
	for (let i = 0; i < line.length - 1; ++i) if (lineIntersectPolygon(line[i], line[i + 1], polygon)) return false;
	return true;
}
function lineStringWithinPolygons(line, polygons) {
	for (let i = 0; i < polygons.length; i++) if (lineStringWithinPolygon(line, polygons[i])) return true;
	return false;
}
...
function coerceValue(type, value) {
	switch (type) {
		...
		case "formatted": return Formatted.fromString(toString(value));
		case "resolvedImage": return ResolvedImage.build(toString(value));
	}
	return value;
}
function clampToAllowedNumber(value, min, max, step) {
	if (step !== void 0) value = step * Math.round(value / step);
	if (min !== void 0 && value < min) value = min;
	if (max !== void 0 && value > max) value = max;
	return value;
}
var Config = class Config {
	...
	static parse(args, context) {
		...
		type ?? (type = ValueType);
		if (args.length < 2 || args.length > 3) return context.error(`Invalid number of arguments for 'config' expression.`);
		const configKey = context.parse(args[1], 1);
		if (!(configKey instanceof Literal)) return context.error(`Key name of 'config' expression must be a string literal.`);
		...
		if (args.length >= 3) {
			...
			if (!(configScope instanceof Literal)) return context.error(`Scope of 'config' expression must be a string literal.`);
		...
		if (context.options) {
			...
			if (config) featureConstant = isFeatureConstant(config.value || config.default);
		}
	...
	evaluate(ctx) {
		...
		if (result !== void 0 && (minValue !== void 0 || maxValue !== void 0 || stepValue !== void 0)) {
			if (typeof result === "number") result = clampToAllowedNumber(result, minValue, maxValue, stepValue);
			else if (Array.isArray(result)) result = result.map((item) => typeof item === "number" ? clampToAllowedNumber(item, minValue, maxValue, stepValue) : item);
		}
		if (value !== void 0 && result !== void 0 && values && !values.includes(result)) {
			result = defaultValue;
			if (type) result = coerceValue(type, result);
		}
		if (type && type !== this.type || result !== void 0 && !typeEquals(typeOf(result), this.type)) result = coerceValue(this.type.kind, result);
		return result;
	}
	eachChild() {}
	outputDefined() {
	...
	serialize() {
		...
		if (this.scope) res.concat(this.scope);
		return res;
	}
};
function isFeatureConstant(e) {
	if (e instanceof CompoundExpression) {
		...
		else if (e.name.startsWith("filter-")) return false;
	}
	if (e instanceof Within) return false;
	if (e instanceof Distance) return false;
	if (e instanceof Config) return e.featureConstant;
	let result = true;
	e.eachChild((arg) => {
...
function lngLatScale(lat) {
	...
	return [m * w * coslat, m * w * w2 * .9933056200098587];
}
...
var MIN_POINT_SIZE = 100;
var MIN_LINE_POINT_SIZE = 50;
function isDefaultBBOX(bbox) {
	...
	if (defualtBBox.length !== bbox.length) return false;
	for (let i = 0; i < defualtBBox.length; i++) if (defualtBBox[i] !== bbox[i]) return false;
	return true;
}
...
function isRangeSafe(range, threshold) {
	...
	if (!ret) console.warn("Distance Expression: Index is out of range");
	return ret;
}
...
function getBBox(pointSets, range) {
	...
	for (let i = range[0]; i <= range[1]; ++i) updateBBox(bbox, pointSets[i]);
	return bbox;
}
function getPolygonBBox(polygon) {
	...
	for (let i = 0; i < polygon.length; ++i) for (let j = 0; j < polygon[i].length; ++j) updateBBox(bbox, polygon[i][j]);
	return bbox;
}
function bboxToBBoxDistance(bbox1, bbox2, kx, ky) {
	if (isDefaultBBOX(bbox1) || isDefaultBBOX(bbox2)) return NaN;
	let dx = 0;
	let dy = 0;
...
function getLngLatPoints(coordinates, canonical) {
	const coords = [];
	for (let i = 0; i < coordinates.length; ++i) coords.push(getLngLatPoint(coordinates[i], canonical));
	return coords;
}
function pointToLineDistance(point, line, kx, ky) {
	return rulerDistance(point, rulerPointOnLine(line, point, kx, ky), kx, ky);
}
function pointsToLineDistance(points, rangeA, line, rangeB, kx, ky) {
	const subLine = line.slice(rangeB[0], rangeB[1] + 1);
	let dist = Infinity;
	for (let i = rangeA[0]; i <= rangeA[1]; ++i) if ((dist = Math.min(dist, pointToLineDistance(points[i], subLine, kx, ky))) === 0) return 0;
	return dist;
}
function segmentToSegmentDistance(p1, p2, q1, q2, kx, ky) {
	const dist1 = Math.min(rulerPointToSegmentDistance(p1, q1, q2, kx, ky), rulerPointToSegmentDistance(p2, q1, q2, kx, ky));
	const dist2 = Math.min(rulerPointToSegmentDistance(q1, p1, p2, kx, ky), rulerPointToSegmentDistance(q2, p1, p2, kx, ky));
	return Math.min(dist1, dist2);
}
function lineToLineDistance(line1, range1, line2, range2, kx, ky) {
	if (!isRangeSafe(range1, line1.length) || !isRangeSafe(range2, line2.length)) return NaN;
	let dist = Infinity;
...
function pointsToPointsDistance(pointSet1, range1, pointSet2, range2, kx, ky) {
	if (!isRangeSafe(range1, pointSet1.length) || !isRangeSafe(range2, pointSet2.length)) return NaN;
	let dist = Infinity;
	for (let i = range1[0]; i <= range1[1]; ++i) for (let j = range2[0]; j <= range2[1]; ++j) if ((dist = Math.min(dist, rulerDistance(pointSet1[i], pointSet2[j], kx, ky))) === 0) return dist;
	return dist;
}
...
function lineToPolygonDistance(line, range, polygon, kx, ky) {
	...
	let dist = Infinity;
	for (let i = range[0]; i < range[1]; ++i) for (const ring of polygon) for (let j = 0, len = ring.length, k = len - 1; j < len; k = j++) {
...
function polygonIntersect(polygon1, polygon2) {
	for (const ring of polygon1) for (let i = 0; i <= ring.length - 1; ++i) if (pointWithinPolygon(ring[i], polygon2, true)) return true;
	return false;
}
function polygonToPolygonDistance(polygon1, polygon2, kx, ky, currentMiniDist = Infinity) {
	const bbox1 = getPolygonBBox(polygon1);
	const bbox2 = getPolygonBBox(polygon2);
	if (currentMiniDist !== Infinity && bboxToBBoxDistance(bbox1, bbox2, kx, ky) >= currentMiniDist) return currentMiniDist;
	...
	for (const ring1 of polygon1) for (let i = 0, len1 = ring1.length, l = len1 - 1; i < len1; l = i++) for (const ring2 of polygon2) for (let j = 0, len2 = ring2.length, k = len2 - 1; j < len2; k = j++) {
...
function pointSetsDistance(pointSet1, isLine1, pointSet2, isLine2, kx, ky, currentMiniDist = Infinity) {
	...
	while (distQueue.length) {
		...
		if (getRangeSize(rangeA) <= set1Threshold && getRangeSize(rangeB) <= set2Threshold) {
			...
			if (isLine1 && isLine2) miniDist = Math.min(miniDist, lineToLineDistance(pointSet1, rangeA, pointSet2, rangeB, kx, ky));
			else if (!isLine1 && !isLine2) miniDist = Math.min(miniDist, pointsToPointsDistance(pointSet1, rangeA, pointSet2, rangeB, kx, ky));
			else if (isLine1 && !isLine2) miniDist = Math.min(miniDist, pointsToLineDistance(pointSet2, rangeB, pointSet1, rangeA, kx, ky));
			else if (!isLine1 && isLine2) miniDist = Math.min(miniDist, pointsToLineDistance(pointSet1, rangeA, pointSet2, rangeB, kx, ky));
			if (miniDist === 0) return miniDist;
		} else {
...
function polygonsToPolygonsDistance(polygons1, polygons2, kx, ky) {
	let dist = Infinity;
	for (const polygon1 of polygons1) for (const polygon2 of polygons2) {
...
function pointsToGeometryDistance(originGeometry, canonical, geometry) {
	const lngLatPoints = [];
	for (const points of originGeometry) for (const point of points) lngLatPoints.push(getLngLatPoint(point, canonical));
	const [kx, ky] = lngLatScale(lngLatPoints[0][1]);
	if (geometry.type === "Point" || geometry.type === "MultiPoint" || geometry.type === "LineString") return pointSetsDistance(lngLatPoints, false, geometry.type === "Point" ? [geometry.coordinates] : geometry.coordinates, geometry.type === "LineString", kx, ky);
	if (geometry.type === "MultiLineString") return pointSetToLinesDistance(lngLatPoints, false, geometry.coordinates, kx, ky);
	if (geometry.type === "Polygon" || geometry.type === "MultiPolygon") return pointSetToPolygonsDistance(lngLatPoints, false, geometry.type === "Polygon" ? [geometry.coordinates] : geometry.coordinates, kx, ky);
	return null;
}
function linesToGeometryDistance(originGeometry, canonical, geometry) {
	...
	for (const line of originGeometry) {
		const lngLatLine = [];
		for (const point of line) lngLatLine.push(getLngLatPoint(point, canonical));
		lngLatLines.push(lngLatLine);
	}
	const [kx, ky] = lngLatScale(lngLatLines[0][0][1]);
	if (geometry.type === "Point" || geometry.type === "MultiPoint" || geometry.type === "LineString") return pointSetToLinesDistance(geometry.type === "Point" ? [geometry.coordinates] : geometry.coordinates, geometry.type === "LineString", lngLatLines, kx, ky);
	...
	if (geometry.type === "Polygon" || geometry.type === "MultiPolygon") {
		...
		for (let i = 0; i < lngLatLines.length; i++) {
			const tempDist = pointSetToPolygonsDistance(lngLatLines[i], true, geometry.type === "Polygon" ? [geometry.coordinates] : geometry.coordinates, kx, ky, dist);
			if (isNaN(tempDist)) return tempDist;
			if ((dist = Math.min(dist, tempDist)) === 0) return dist;
		}
		return dist;
...
function polygonsToGeometryDistance(originGeometry, canonical, geometry) {
	...
	for (const polygon of classifyRings(originGeometry)) {
		const lngLatPolygon = [];
		for (let i = 0; i < polygon.length; ++i) lngLatPolygon.push(getLngLatPoints(polygon[i], canonical));
		lngLatPolygons.push(lngLatPolygon);
	}
	const [kx, ky] = lngLatScale(lngLatPolygons[0][0][0][1]);
	if (geometry.type === "Point" || geometry.type === "MultiPoint" || geometry.type === "LineString") return pointSetToPolygonsDistance(geometry.type === "Point" ? [geometry.coordinates] : geometry.coordinates, geometry.type === "LineString", lngLatPolygons, kx, ky);
	if (geometry.type === "MultiLineString") {
		let dist = Infinity;
		for (let i = 0; i < geometry.coordinates.length; i++) {
	...
	if (geometry.type === "Polygon" || geometry.type === "MultiPolygon") return polygonsToPolygonsDistance(geometry.type === "Polygon" ? [geometry.coordinates] : geometry.coordinates, lngLatPolygons, kx, ky);
	return null;
}
...
function extractDistanceGeometry(value) {
	if (!isValue(value) || typeof value !== "object" || value === null || Array.isArray(value)) return null;
	const geojson = value;
	...
	if (geojson.type === "Feature") return isTypeValid(geojson.geometry.type) ? [geojson.geometry] : null;
	if (isTypeValid(geojson.type)) return [geojson];
	return null;
}
var Distance = class Distance {
	...
	static parse(args, context) {
		if (args.length !== 2) return context.error(`'distance' expression requires either one argument, but found ' ${args.length - 1} instead.`);
		const arg = args[1];
		...
		]) if (!isGlobalPropertyConstant(parsed, globalProperties)) return context.error(`'distance' expression may not depend on ${name}.`);
		if (!isStateConstant(parsed)) return context.error(`'distance' expression may not depend on feature-state.`);
		if (parsed instanceof Literal && !extractDistanceGeometry(parsed.value)) return context.error("'distance' expression needs to be an array with format ['Distance', GeoJSONObj].");
...
var ParsingContext = class ParsingContext {
	...
	*/
	_forkForSignature() {
		return new ParsingContext(this.registry, this.path.slice(), null, this.scope, [], this._scope, this.options, this.iconImageUseTheme);
	}
	/**
...
var CONSTANT_FOLD_EXCLUDED_GLOBALS = /* @__PURE__ */ new Set([
	"zoom",
	"heatmap-density",
	"worldview",
	"line-progress",
...
function isConstant(expression) {
	...
	else if (expression instanceof Config) return false;
...
var Step = class Step {
	...
	static parse(args, context) {
		...
		for (let i = 1; i < args.length; i += 2) {
			...
			if (stops.length && stops.at(-1)[0] >= label) return context.error("Input/output pairs for \"step\" expressions must be arranged with input values in strictly ascending order.", labelKey);
...
function color(from, to, t) {
	return new Color(lerp(from.r, to.r, t), lerp(from.g, to.g, t), lerp(from.b, to.b, t), lerp(from.a, to.a, t));
}
function array(from, to, t) {
	return from.map((d, i) => {
		return lerp(d, to[i], t);
	});
}
...
function rgbToLab(rgbColor) {
	const b = rgb2xyz(rgbColor.r), a = rgb2xyz(rgbColor.g), l = rgb2xyz(rgbColor.b), x = xyz2lab((.4124564 * b + .3575761 * a + .1804375 * l) / Xn), y = xyz2lab((.2126729 * b + .7151522 * a + .072175 * l) / Yn), z = xyz2lab((.0193339 * b + .119192 * a + .9503041 * l) / Zn);
	return {
		l: 116 * y - 16,
		a: 500 * (x - y),
		b: 200 * (y - z),
...
function labToRgb(labColor) {
	...
	x = Xn * lab2xyz(x);
	z = Zn * lab2xyz(z);
	return new Color(xyz2rgb(3.2404542 * x - 1.5371385 * y - .4985314 * z), xyz2rgb(-.969266 * x + 1.8760108 * y + .041556 * z), xyz2rgb(.0556434 * x - .2040259 * y + 1.0572252 * z), labColor.alpha);
}
...
var Interpolate = class Interpolate {
	...
	static parse(args, context) {
		...
		if (args.length - 1 < 4) return context.error(`Expected at least 4 arguments, but found only ${args.length - 1}.`);
		if (args.length - 1 > 3 && (args.length - 1) % 2 !== 0) return context.error(`Expected an even number of arguments.`);
		...
		for (let i = 0; i < rest.length; i += 2) {
			...
			if (stops.length && stops.at(-1)[0] >= label) return context.error("Input/output pairs for \"interpolate\" expressions must be arranged with input values in strictly ascending order.", labelKey);
			const parsed = context.parse(value, valueKey, outputType);
			if (!parsed) return null;
			outputType = outputType || parsed.type;
			stops.push([label, parsed]);
	...
	serialize() {
		let interpolation;
		if (this.interpolation.name === "linear") interpolation = ["linear"];
		else if (this.interpolation.name === "exponential") {
			if (this.interpolation.base === 1) interpolation = ["linear"];
			else interpolation = ["exponential", this.interpolation.base];
		} else interpolation = ["cubic-bezier", ...this.interpolation.controlPoints];
		const serialized = [
			this.operator,
			interpolation,
...
var INVALID_VAR_CHAR_RE = /[^a-zA-Z0-9_]/;
var Let = class Let {
	...
	static parse(args, context) {
		...
		for (let i = 1; i < args.length - 1; i += 2) {
			...
			if (INVALID_VAR_CHAR_RE.test(name)) return context.error(`Variable names must contain only alphanumeric characters or '_'.`, i);
...
var At = class At {
	...
	evaluate(ctx) {
		...
		if (index < 0) throw new RuntimeError(`Array index out of bounds: negative index`);
		if (index >= array2.length) throw new RuntimeError(`Array index out of bounds: index exceeds array size`);
		if (index !== Math.floor(index)) throw new RuntimeError(`Array index must be an integer. Use at-interpolated for fractional indices`);
		return array2[index];
	}
	eachChild(fn) {
		fn(this.index);
...
var AtInterpolated = class AtInterpolated {
	constructor(type, index, input) {
		this.type = type;
		this.index = index;
		this.input = input;
	...
	evaluate(ctx) {
		...
		if (index > array2.length - 1) throw new RuntimeError(`Array index out of bounds: ${index} > ${array2.length - 1}.`);
		if (index === Math.floor(index)) return array2[index];
		const lowerIndex = Math.floor(index);
		const upperIndex = Math.ceil(index);
		const lowerValue = array2[lowerIndex];
		const upperValue = array2[upperIndex];
		if (typeof lowerValue !== "number" || typeof upperValue !== "number") throw new RuntimeError(`Cannot interpolate between non-number values at index ${index}.`);
		return lerp(lowerValue, upperValue, index - lowerIndex);
	}
	eachChild(fn) {
		fn(this.index);
		fn(this.input);
...
var Match = class Match {
	...
	static parse(args, context) {
		...
		for (let i = 2; i < args.length - 1; i += 2) {
			...
			if (labels.length === 0) return context.error("Expected at least one branch label.", i);
			for (const label of labels) {
				if (typeof label !== "number" && typeof label !== "string") return context.error(`Branch labels must be numbers or strings.`, i);
				else if (typeof label === "number" && Math.abs(label) > Number.MAX_SAFE_INTEGER) return context.error(`Branch labels must be integers no larger than ${Number.MAX_SAFE_INTEGER}.`, i);
				else if (typeof label === "number" && Math.floor(label) !== label) return context.error(`Numeric branch labels must be integer values.`, i);
				else if (!inputType) inputType = typeOf(label);
				else if (context.checkSubtype(inputType, typeOf(label), i)) return null;
				if (typeof cases[String(label)] !== "undefined") return context.error("Branch labels must be unique.", i);
		...
		if (input.type.kind !== "value" && context.checkSubtype(inputType, input.type, 1)) return null;
		return new Match(inputType, outputType, input, cases, outputs, otherwise);
	}
	evaluate(ctx) {
		const input = this.input.evaluate(ctx);
		return (typeEquals(typeOf(input), this.inputType) && this.outputs[this.cases[input]] || this.otherwise).evaluate(ctx);
	}
	eachChild(fn) {
		fn(this.input);
	...
	serialize() {
		...
		for (const label of sortedLabels) {
			...
			} else groupedByOutput[outputIndex][1].push(label);
		}
...
var Slice = class Slice {
	...
	evaluate(ctx) {
		...
		if (!isValidNativeType(input, ["string", "array"])) throw new RuntimeError(`Expected first argument to be of type array or string, but found ${toString$1(typeOf(input))} instead.`);
		if (this.endIndex) {
...
function makeComparison(op, compareBasic, compareWithCollator) {
	...
	return class Comparison {
		...
		static parse(args, context) {
			...
			if (!lhs) return null;
			if (!isComparableType(op2, lhs.type)) return context.error(`"${op2}" comparisons are not supported for type '${toString$1(lhs.type)}'.`, 1);
			let rhs = context.parse(args[2], 2, ValueType);
			if (!rhs) return null;
			if (!isComparableType(op2, rhs.type)) return context.error(`"${op2}" comparisons are not supported for type '${toString$1(rhs.type)}'.`, 2);
		...
		evaluate(ctx) {
			...
			if (isOrderComparison && this.hasUntypedArgument) {
				...
				const rt = typeOf(rhs);
				if (lt2.kind !== rt.kind || !(lt2.kind === "string" || lt2.kind === "number")) throw new RuntimeError(`Expected arguments for "${op}" to be (string, string) or (number, number), but found (${lt2.kind}, ${rt.kind}) instead.`);
			}
...
var Length = class Length {
	...
	evaluate(ctx) {
		...
		else throw new RuntimeError(`Expected value to be of type string or array, but found ${toString$1(typeOf(input))} instead.`);
	}
	eachChild(fn) {
		fn(this.input);
	}
...
function assertNeedleHaystack(needle, haystack) {
	...
	if (!isValidNativeType(haystack, ["string", "array"])) throw new RuntimeError(`Expected second argument to be of type array or string, but found ${toString$1(typeOf(haystack))} instead.`);
}
...
CompoundExpression.register(expressions, {
	...
	"is-active-floor": [
		...
		varargs(StringType),
		(ctx, args) => {
			if (!(ctx.globals && ctx.globals.activeFloors && ctx.globals.activeFloors.size > 0)) return false;
	...
	"index-of": {
		type: NumberType,
		overloads: [[[ValueType, ValueType], (ctx, [needleExpr, haystackExpr]) => {
			const needle = needleExpr.evaluate(ctx);
			const haystack = haystackExpr.evaluate(ctx);
			assertNeedleHaystack(needle, haystack);
	...
	"random": [
		...
		(ctx, args) => {
			...
			if (min === max) return min;
			let seedVal;
			if (typeof seed === "string") seedVal = hashString(seed);
			else if (typeof seed === "number") seedVal = seed;
			else throw new RuntimeError(`Invalid seed input: ${seed}`);
			return min + mulberry32(seedVal)() * (max - min);
		}
	]
});
function getConfigDependencies(e) {
	if (e instanceof Config) return /* @__PURE__ */ new Set([e.key]);
...
function isIndoorDependent(e) {
	if (e instanceof CompoundExpression && e.name === "is-active-floor") return true;
	let result = false;
	e.eachChild((arg) => {
		if (!result && isIndoorDependent(arg)) result = true;
	});
	return result;
}
...
var TRANSITION_KEY_RE = /^(.*)-transition$/;
var USE_THEME_KEY_RE = /^(.*)-use-theme$/;
...
function evaluateIntervalFunction(parameters, propertySpec, input) {
	if (!isNumber(input)) return coalesce$1(parameters.default, propertySpec.default);
...
function evaluateIdentityFunction(parameters, propertySpec, input) {
	...
	else if (propertySpec.type === "resolvedImage") input = ResolvedImage.build(input.toString());
	else if (getType(input) !== propertySpec.type && (propertySpec.type !== "enum" || !propertySpec.values[input])) input = void 0;
...
var StyleExpression = class {
	...
	evaluate(globals, feature, featureState, canonical, availableImages, formattedSection, featureTileCoord, featureDistanceData, iconImageUseTheme) {
		if (!this._evaluator) this._evaluator = new EvaluationContext(this._scope, this._options, this._iconImageUseTheme);
		...
		try {
			...
			if (!this._warningHistory[error2.message]) {
				...
				if (typeof console !== "undefined") console.warn(`Failed to evaluate expression "${JSON.stringify(this.expression.serialize())}". ${error2.message}`);
			}
...
function createPropertyExpression(expression, propertySpec, scope, options, iconImageUseTheme) {
	...
	if (!isFeatureConstant$1 && !supportsPropertyExpression(propertySpec)) return error([new ParsingError$1("", "data expressions not supported")]);
	const isZoomConstant = isGlobalPropertyConstant(parsed, [
		"zoom",
		"pitch",
		"distance-from-center"
	]);
	if (!isZoomConstant && !supportsZoomExpression(propertySpec)) return error([new ParsingError$1("", "zoom expressions not supported")]);
	const isLightConstant = isGlobalPropertyConstant(parsed, ["measure-light"]);
	if (!isLightConstant && !supportsLightExpression(propertySpec)) return error([new ParsingError$1("", "measure-light expression not supported")]);
	const isLineProgressConstant = isGlobalPropertyConstant(parsed, ["line-progress"]);
	if (!isLineProgressConstant && !supportsLineProgressExpression(propertySpec)) return error([new ParsingError$1("", "line-progress expression not supported")]);
	const canRelaxZoomRestriction = propertySpec.expression && propertySpec.expression.relaxZoomRestriction;
	const zoomCurve = findZoomCurve(parsed);
	if (!zoomCurve && !isZoomConstant && !canRelaxZoomRestriction) return error([new ParsingError$1("", "\"zoom\" expression may only be used as input to a top-level \"step\" or \"interpolate\" expression, or in the properties of atmosphere.")]);
	else if (zoomCurve instanceof ParsingError$1) return error([zoomCurve]);
	else if (zoomCurve instanceof Interpolate && !supportsInterpolation(propertySpec)) return error([new ParsingError$1("", "\"interpolate\" expressions cannot be used with this property")]);
	if (!zoomCurve) return success(isFeatureConstant$1 && isLineProgressConstant ? new ZoomConstantExpression("constant", expression.value, isLightConstant, isLineProgressConstant) : new ZoomConstantExpression("source", expression.value, isLightConstant, isLineProgressConstant));
...
function getDefaultValue(spec) {
	if (spec.type === "color" && (isFunction(spec.default) || Array.isArray(spec.default))) return new Color(0, 0, 0, 0);
	else if (spec.type === "color") return Color.parse(spec.default) || null;
	else if (spec.default === void 0) return null;
...
function deepUnbundle(value) {
	if (Array.isArray(value)) return value.map(deepUnbundle);
		...
		const valueRec = value;
		for (const key in valueRec) unbundledValue[key] = deepUnbundle(valueRec[key]);
...
var ValidationWarning = class extends ValidationError {};
var LINE_NUMBER_RE = /line (\d+)/;
...
function validateImport(options) {
	...
	if (!isObject(value)) return [new ValidationError(key, value, `import must be an object`)];
...
function validateOption(options) {
	...
	const declaredType = !(isObject(optionValue) && unbundle(optionValue.array) === true) && isObject(optionValue) ? unbundle(optionValue.type) : void 0;
	const validateDefault = (elementOptions) => {
		const defaultValue = elementOptions.value;
		if (isObject(defaultValue)) return validate({
			...elementOptions,
			valueSpec: {
				...elementOptions.valueSpec,
				type: "*",
				expression: void 0
			}
		});
		if (declaredType && Array.isArray(defaultValue)) return validate({
			...elementOptions,
			valueSpec: {
				...elementOptions.valueSpec,
				type: declaredType
	...
	return validateObject({
		...
		objectElementValidators: { default: validateDefault }
	});
}
function validateArray(options) {
	...
	if (!Array.isArray(array)) return [new ValidationError(key, array, `array expected, ${getType(array)} found`)];
	...
	if (arraySpec["min-length"] && array.length < arraySpec["min-length"]) return [new ValidationError(key, array, `array length at least ${arraySpec["min-length"]} expected, length ${array.length} found`)];
	...
	if (isObject(arraySpec.value)) arrayElementSpec = arraySpec.value;
	let errors = [];
...
function validateNumber(options) {
	...
	if (!isNumber(value)) return [new ValidationError(key, value, `number expected, ${getType(value)} found`)];
	if (value !== value) return [new ValidationError(key, value, `number expected, NaN found`)];
	if ("minimum" in valueSpec) {
		...
		if (value < specMin) return [new ValidationError(key, value, `${value} is less than the minimum value ${specMin}`)];
	}
	if ("maximum" in valueSpec) {
		...
		if (value > specMax) return [new ValidationError(key, value, `${value} is greater than the maximum value ${specMax}`)];
	}
	return [];
}
...
function validateFunction(options) {
	const key = options.key;
	const value = options.value;
	if (!isObject(value)) return [new ValidationError(key, value, `object expected, ${getType(value)} found`)];
	...
	if (functionType !== "identity" && !value.stops) errors.push(new ValidationError(options.key, options.value, "missing required property \"stops\""));
	if (functionType === "exponential" && functionValueSpec.expression && !supportsInterpolation(functionValueSpec)) errors.push(new ValidationError(options.key, options.value, "exponential functions not supported"));
	if (options.styleSpec.$version >= 8) {
		if (isPropertyFunction && !supportsPropertyExpression(functionValueSpec)) errors.push(new ValidationError(options.key, options.value, "property functions not supported"));
		else if (isZoomFunction && !supportsZoomExpression(functionValueSpec)) errors.push(new ValidationError(options.key, options.value, "zoom functions not supported"));
	}
	if ((functionType === "categorical" || isZoomAndPropertyFunction) && value.property === void 0) errors.push(new ValidationError(options.key, options.value, "\"property\" property is required"));
	return errors;
	function validateFunctionStops(options2) {
		if (functionType === "identity") return [new ValidationError(options2.key, options2.value, "identity function may not have a \"stops\" property")];
		let errors2 = [];
		...
		if (Array.isArray(value2) && value2.length === 0) errors2.push(new ValidationError(options2.key, value2, "array must have at least one stop"));
		return errors2;
	}
	function validateFunctionStop(options2) {
		...
		if (!Array.isArray(value2)) return [new ValidationError(key2, value2, `array expected, ${getType(value2)} found`)];
		if (value2.length !== 2) return [new ValidationError(key2, value2, `array length 2 expected, length ${value2.length} found`)];
		if (isZoomAndPropertyFunction) {
			if (!isObject(value2[0])) return [new ValidationError(key2, value2, `object expected, ${getType(value2[0])} found`)];
			const stopKey = value2[0];
			if (stopKey.zoom === void 0) return [new ValidationError(key2, value2, "object stop key must have zoom")];
			if (stopKey.value === void 0) return [new ValidationError(key2, value2, "object stop key must have value")];
			const nextStopDomainZoom = unbundle(stopKey.zoom);
			if (typeof nextStopDomainZoom !== "number") return [new ValidationError(key2, stopKey.zoom, "stop zoom values must be numbers")];
			if (previousStopDomainZoom && previousStopDomainZoom > nextStopDomainZoom) return [new ValidationError(key2, stopKey.zoom, "stop zoom values must appear in ascending order")];
		...
		} else errors2 = errors2.concat(validateStopDomainValue({
			key: `${key2}[0]`,
			value: value2[0],
			style: options2.style,
			styleSpec: options2.styleSpec
		}, value2));
		if (isExpression(deepUnbundle(value2[1]))) return errors2.concat([new ValidationError(`${key2}[1]`, value2[1], "expressions are not allowed in function stops.")]);
	...
	function validateStopDomainValue(options2, stop) {
		...
		else if (type !== stopKeyType) return [new ValidationError(options2.key, reportValue, `${type} stop domain type must match previous stop domain type ${stopKeyType}`)];
		if (type !== "number" && type !== "string" && type !== "boolean" && typeof value2 !== "number" && typeof value2 !== "string" && typeof value2 !== "boolean") return [new ValidationError(options2.key, reportValue, "stop domain value must be a number, string, or boolean")];
		...
		if (functionType === "categorical" && type === "number" && (typeof value2 !== "number" || !isFinite(value2) || Math.floor(value2) !== value2)) return [new ValidationError(options2.key, reportValue, `integer expected, found ${String(value2)}`)];
		if (functionType !== "categorical" && type === "number" && typeof value2 === "number" && typeof previousStopDomainValue === "number" && previousStopDomainValue !== void 0 && value2 < previousStopDomainValue) return [new ValidationError(options2.key, reportValue, "stop domain values must appear in ascending order")];
		else previousStopDomainValue = value2;
		if (functionType === "categorical" && value2 in stopDomainValues) return [new ValidationError(options2.key, reportValue, "stop domain values must be unique")];
		else stopDomainValues[value2] = true;
		return [];
	}
...
function validateExpression(options) {
	...
	if (options.expressionContext === "appearance") return checkDisallowedAppearancesParameters(expressionObj, options);
...
function disallowedFilterParameters(e, options) {
	...
	if (options.valueSpec && options.valueSpec.expression) for (const param of options.valueSpec.expression.parameters) disallowedParameters.delete(param);
	if (disallowedParameters.size === 0) return [];
	const errors = [];
	if (e instanceof CompoundExpression) {
		if (disallowedParameters.has(e.name)) return [new ValidationError(options.key, options.value, `["${e.name}"] expression is not supported in a filter for a ${options.object.type} layer with id: ${options.object.id}`)];
	}
	e.eachChild((arg) => {
...
var FEATURE_OPERATORS = /* @__PURE__ */ new Set([
	"get",
	"has",
	"properties",
	"geometry-type",
...
var APPEARANCES_DISALLOWED_PARAMETERS = /* @__PURE__ */ new Set([
	"zoom",
	"pitch",
	"distance-from-center",
	"feature",
...
function checkDisallowedAppearancesParameters(e, options) {
	if (options.valueSpec && options.valueSpec.expression) for (const param of options.valueSpec.expression.parameters) APPEARANCES_DISALLOWED_PARAMETERS.delete(param);
	if (APPEARANCES_DISALLOWED_PARAMETERS.size === 0) return [];
	const errors = [];
	if (e instanceof CompoundExpression) {
		const param = FEATURE_OPERATORS.has(e.name) ? "feature" : e.name;
		if (APPEARANCES_DISALLOWED_PARAMETERS.has(param)) return [new ValidationError(options.key, options.value, `["${e.name}"] is not an allowed parameter`)];
	}
	e.eachChild((arg) => {
...
function validateBoolean(options) {
	const value = options.value;
	const key = options.key;
	if (!isBoolean(value)) return [new ValidationError(key, value, `boolean expected, ${getType(value)} found`)];
	return [];
}
function validateColor({ key, value }) {
	if (!isString(value)) return [new ValidationError(key, value, `color expected, ${getType(value)} found`)];
	if (Color.parse(unbundle(value)) === void 0) return [new ValidationError(key, value, `color expected, "${value}" found`)];
	return [];
}
function validateEnum(options) {
	...
	if (Array.isArray(valueSpec.values)) {
		if (!valueSpec.values.includes(unbundle(value))) errors.push(new ValidationError(key, value, `expected one of [${valueSpec.values.join(", ")}], ${JSON.stringify(value)} found`));
	} else if (!Object.keys(valueSpec.values).includes(unbundle(value))) errors.push(new ValidationError(key, value, `expected one of [${Object.keys(valueSpec.values).join(", ")}], ${JSON.stringify(value)} found`));
	return errors;
}
...
function validateNonExpressionFilter(options) {
	const value = options.value;
	const key = options.key;
	if (!Array.isArray(value)) return [new ValidationError(key, value, `array expected, ${getType(value)} found`)];
	...
	const validate = () => {
		if (value.length >= 2) {
			if (!isString(value[1])) errors.push(new ValidationError(`${key}[1]`, value[1], `string expected, ${getType(value[1])} found`));
		}
		for (let i = 2; i < value.length; i++) if (unbundle(value[1]) === "$type") errors = errors.concat(validateEnum({
			key: `${key}[${i}]`,
			value: value[i],
			valueSpec: styleSpec.geometry_type
		}));
		else if (!isString(value[i]) && !isNumber(value[i]) && !isBoolean(value[i])) errors.push(new ValidationError(`${key}[${i}]`, value[i], `string, number, or boolean expected, ${getType(value[i])} found.`));
	};
	switch (unbundle(value[0])) {
		...
		case "none":
			...
			break;
		case "has":
		case "!has": if (value.length !== 2) errors.push(new ValidationError(key, value, `filter array for "${value[0]}" operator must have 2 elements`));
		else if (!isString(value[1])) errors.push(new ValidationError(`${key}[1]`, value[1], `string expected, ${getType(value[1])} found`));
	}
	return errors;
}
var TOKEN_PATTERN_RE = /^{([^}]+)}$/;
function validateProperty(options, propertyType) {
	...
	if (useThemeMatch && layerSpec[useThemeMatch[1]]) {
		if (isExpression(deepUnbundle(value))) return [].concat(validate({
			key,
			value,
			valueSpec: {
				type: "string",
	...
	if (!valueSpec) return [new ValidationWarning(key, value, `unknown property "${propertyKey}"`)];
	let tokenMatch;
	if (isString(value) && supportsPropertyExpression(valueSpec) && !valueSpec.tokens && (tokenMatch = TOKEN_PATTERN_RE.exec(value))) return [new ValidationError(key, value, `"${propertyKey}" does not support interpolation syntax
Use an identity property function instead: ${`\`{ "type": "identity", "property": ${tokenMatch ? JSON.stringify(tokenMatch[1]) : "\"_\""} }\``}.`)];
	const errors = [];
	if (options.layerType === "symbol") {
		if (propertyKey === "text-field" && style && !style.glyphs && !style.imports) errors.push(new ValidationError(key, value, "use of \"text-field\" requires a style \"glyphs\" property"));
		...
		if (supportsPropertyExpression(valueSpec) && (supportsLightExpression(valueSpec) || supportsZoomExpression(valueSpec))) {
			const expressionValue = createPropertyExpression(deepUnbundle(value), valueSpec).value;
			...
			if (expressionObj && !isGlobalPropertyConstant(expressionObj, ["measure-light"])) {
				if (propertyKey !== "model-emissive-strength" || !isFeatureConstant(expressionObj) || !isStateConstant(expressionObj)) errors.push(new ValidationError(key, value, `${propertyKey} does not support measure-light expressions when the model layer source is vector tile or GeoJSON.`));
			}
		}
	}
...
function validateAppearance(options) {
	...
	const errors = validateObject({
		...
		objectElementValidators: {
			...
			properties: (options2) => validateProperties({
				layer,
				layerType,
				...options2
			})
	...
	if (name !== "hidden" && condition === void 0) errors.push(new ValidationError(options.key, "name", `Appearance with name different than "hidden" must have a condition`));
	return errors;
}
...
function validateCondition(options) {
	const errors = [];
	const condition = options.object.condition;
...
function validateLayer(options) {
	...
	const style = options.style;
	const styleSpec = options.styleSpec;
	if (!isObject(layer)) return [new ValidationError(key, layer, `object expected`)];
	...
	if (layer.id) {
		...
		for (let i = 0; i < options.arrayIndex; i++) {
			...
			if (unbundle(otherLayer.id) === layerId) errors.push(new ValidationError(key, layer.id, `duplicate layer id "${layerId}", previously used at line ${otherLayer.id.__line__}`));
		}
	}
	if ("ref" in layer) {
		[
		...
		style.layers.forEach((layer2) => {
			if (unbundle(layer2.id) === ref) parent = layer2;
		});
		if (!parent) {
			if (typeof ref === "string") errors.push(new ValidationError(key, layer.ref, `ref layer "${ref}" not found`));
		...
		if (!layer.source) errors.push(new ValidationError(key, layer, "missing required property \"source\""));
		else if (!isString(layer.source)) errors.push(new ValidationError(`${key}.source`, layer.source, "\"source\" must be a string"));
		else {
			const source = style.sources && Object.hasOwn(style.sources, layer.source) ? style.sources[layer.source] : void 0;
			...
			else if (sourceType === "raster-dem" && type !== "hillshade") errors.push(new ValidationError(key, layer.source, "raster-dem source can only be used with layer type 'hillshade'."));
			else if (sourceType === "raster-array" && !["raster", "raster-particle"].includes(type)) errors.push(new ValidationError(key, layer.source, `raster-array source can only be used with layer type 'raster'.`));
			else if (type === "line" && layer.paint && layer.paint["line-gradient"] && sourceType === "geojson" && !source.lineMetrics) errors.push(new ValidationError(key, layer, `layer "${layer.id}" specifies a line-gradient, which requires the GeoJSON source to have \`lineMetrics\` enabled.`));
			else if (type === "line" && layer.paint && layer.paint["line-trim-offset"] && sourceType === "geojson" && !source.lineMetrics) errors.push(new ValidationError(key, layer, `layer "${layer.id}" specifies a line-trim-offset, which requires the GeoJSON source to have \`lineMetrics\` enabled.`));
			else if (type === "raster-particle" && sourceType !== "raster-array") errors.push(new ValidationError(key, layer.source, `layer "${layer.id}" requires a 'raster-array' source.`));
		}
	}
		...
		objectElementValidators: {
			...
			layout(options2) {
				return validateObject({
					...
					style: options2.style,
					styleSpec: options2.styleSpec,
					objectElementValidators: { "*"(options3) {
			...
			appearances(options2) {
				const validationErrors = validateArray({
					...
					style: options2.style,
					styleSpec: options2.styleSpec,
					arrayElementValidator: (options3) => validateAppearance({
						layerType: type,
						layer,
						...options3
					})
...
function validateSource(options) {
	...
	const style = options.style;
	if (!isObject(value)) return [new ValidationError(key, value, `object expected, ${getType(value)} found`)];
	if (!("type" in value)) return [new ValidationError(key, value, "\"type\" is required")];
	...
	if ([
		...
		if (!("url" in value) && !("tiles" in value)) errors.push(new ValidationWarning(key, value, "Either \"url\" or \"tiles\" is required."));
	}
	switch (type) {
		case "vector":
		case "raster":
		...
		case "geojson":
			...
			if ("cluster" in value && "clusterProperties" in value) {
				if (!isObject(value.clusterProperties)) return [new ValidationError(`${key}.clusterProperties`, value, `object expected, ${getType(value)} found`)];
				for (const prop in value.clusterProperties) {
					const propValue = value.clusterProperties[prop];
					if (!Array.isArray(propValue)) return [new ValidationError(`${key}.clusterProperties.${prop}`, propValue, "array expected")];
...
function getSourceTypeValues(styleSpec) {
	return styleSpec.source.reduce((memo, source) => {
		...
		if (sourceType.type.type === "enum") memo = memo.concat(Object.keys(sourceType.type.values || {}));
		return memo;
	}, []);
}
function validatePromoteId({ key, value }) {
	if (isString(value)) return validateString({
		key,
		value
	});
	if (Array.isArray(value)) {
		const errors2 = [];
		const expression = createExpression(deepUnbundle(value));
		...
		const parsed = expression.value.expression;
		if (!isGlobalPropertyConstant(parsed, [
			"zoom",
			"heatmap-density",
			"line-progress",
			"raster-value",
	...
	if (!isObject(value)) return [new ValidationError(key, value, `string, expression or object expected, "${getType(value)}" found`)];
	const errors = [];
	for (const prop in value) errors.push(...validatePromoteId({
		key: `${key}.${prop}`,
		value: value[prop]
	}));
	return errors;
...
function validateModel(options) {
	...
	if (!url) return [];
	if (!isString(url)) return [new ValidationError(options.key, url, `string expected, "${getType(url)}" found`)];
	if (!isValidUrl(url, true)) return [new ValidationError(options.key, url, `invalid url "${url}"`)];
	return [];
}
function validateLight(options) {
	...
	if (!isObject(light)) return [new ValidationError("light", light, `object expected, ${getType(light)} found`)];
	let errors = [];
	for (const key in light) {
		const transitionMatch = key.match(TRANSITION_KEY_RE);
		const useThemeMatch = key.match(USE_THEME_KEY_RE);
		if (useThemeMatch && lightSpec[useThemeMatch[1]]) errors = errors.concat(validate({
			key,
			value: light[key],
			valueSpec: { type: "string" },
			style,
		...
		else if (transitionMatch && lightSpec[transitionMatch[1]] && lightSpec[transitionMatch[1]].transition) errors = errors.concat(validate({
			key,
			value: light[key],
			valueSpec: styleSpec.transition,
			style,
...
function validateLights(options) {
	...
	if (!light) return [];
	const key = options.key;
	if (!isObject(light)) return [new ValidationError(key, light, `object expected, ${getType(light)} found`)];
	let errors = [];
	const styleSpec = options.styleSpec;
	const lightSpec = styleSpec["light-3d"];
	const style = options.style;
	const lights = options.style.lights;
	for (const prop of ["type", "id"]) if (!(prop in light)) {
	...
	if (lights) for (let i = 0; i < options.arrayIndex; i++) {
		const lightType2 = unbundle(light.type);
		const otherLight = lights[i];
		if (unbundle(otherLight.type) === lightType2) errors.push(new ValidationError(key, light.id, `duplicate light type "${light.type}", previously defined at line ${otherLight.id.__line__}`));
	}
	...
	for (const key2 in light) if (key2 === "properties") {
		...
		for (const propertyKey in properties) {
			const transitionMatch = propertyKey.match(TRANSITION_KEY_RE);
			const useThemeMatch = propertyKey.match(USE_THEME_KEY_RE);
			if (useThemeMatch && lightPropertySpec[useThemeMatch[1]]) errors = errors.concat(validate({
				key: propertyKey,
				value: properties[propertyKey],
				valueSpec: { type: "string" },
				style,
			...
			else if (transitionMatch && lightPropertySpec[transitionMatch[1]] && lightPropertySpec[transitionMatch[1]].transition) errors = errors.concat(validate({
				key: propertyKey,
				value: properties[propertyKey],
				valueSpec: styleSpec.transition,
				style,
			...
			else if (!lightPropertySpec[propertyKey]) errors = errors.concat([new ValidationWarning(options.key, properties[propertyKey], `unknown property "${propertyKey}"`)]);
			else errors = errors.concat(validate({
				key: propertyKey,
				value: properties[propertyKey],
	...
	} else if (lightSpec[key2]) errors = errors.concat(validate({
		key: key2,
		value: light[key2],
	...
	else errors = errors.concat([new ValidationWarning(key2, light[key2], `unknown property "${key2}"`)]);
	return errors;
}
function validateTerrain(options) {
	...
	if (!isObject(terrain)) return [new ValidationError("terrain", terrain, `object expected, ${getType(terrain)} found`)];
	let errors = [];
	for (const key2 in terrain) {
		...
		if (useThemeMatch && terrainSpec[useThemeMatch[1]]) errors = errors.concat(validate({
			key: key2,
			value: terrain[key2],
			valueSpec: { type: "string" },
			style,
		...
		else if (transitionMatch && terrainSpec[transitionMatch[1]] && terrainSpec[transitionMatch[1]].transition) errors = errors.concat(validate({
			key: key2,
			value: terrain[key2],
			valueSpec: styleSpec.transition,
			style,
		...
		else if (terrainSpec[key2]) errors = errors.concat(validate({
			key: key2,
			value: terrain[key2],
		...
		else errors = errors.concat([new ValidationWarning(key2, terrain[key2], `unknown property "${key2}"`)]);
	}
	if (!terrain.source) errors.push(new ValidationError(key, terrain, `terrain is missing required property "source"`));
	else if (!isString(terrain.source)) errors.push(new ValidationError(`${key}.source`, terrain.source, `source must be a string`));
	else {
		const source = style.sources && Object.hasOwn(style.sources, terrain.source) ? style.sources[terrain.source] : void 0;
		const sourceType = source && unbundle(source.type);
		if (!source) errors.push(new ValidationError(`${key}.source`, terrain.source, `source "${terrain.source}" not found`));
		else if (sourceType !== "raster-dem") errors.push(new ValidationError(`${key}.source`, terrain.source, `terrain cannot be used with a source of type ${sourceType}, it only be used with a "raster-dem" source type`));
	}
	return errors;
}
function validateFog(options) {
	...
	if (fog === void 0) return [];
	if (!isObject(fog)) return [new ValidationError("fog", fog, `object expected, ${getType(fog)} found`)];
	let errors = [];
	for (const key in fog) {
		const transitionMatch = key.match(TRANSITION_KEY_RE);
		const useThemeMatch = key.match(USE_THEME_KEY_RE);
		if (useThemeMatch && fogSpec[useThemeMatch[1]]) errors = errors.concat(validate({
			key,
			value: fog[key],
			valueSpec: { type: "string" },
			style,
		...
		else if (transitionMatch && fogSpec[transitionMatch[1]] && fogSpec[transitionMatch[1]].transition) errors = errors.concat(validate({
			key,
			value: fog[key],
			valueSpec: styleSpec.transition,
			style,
		...
		else errors = errors.concat([new ValidationWarning(key, fog[key], `unknown property "${key}"`)]);
	}
	return errors;
}
...
function validateProjection(options) {
	...
	if (isObject(projection)) {
		let errors = [];
		for (const key in projection) errors = errors.concat(validate({
			key,
	...
	if (!isString(projection)) return [new ValidationError("projection", projection, `object or string expected, ${getType(projection)} found`)];
	return [];
}
...
function validateIconset(options) {
	...
	const styleSpec = options.styleSpec;
	const style = options.style;
	if (!isObject(iconset)) return [new ValidationError(key, iconset, "object expected")];
	if (!iconset.type) return [new ValidationError(key, iconset, "\"type\" is required")];
	...
	if (isSourceIconset(type, iconset)) {
		const source = style.sources && Object.hasOwn(style.sources, iconset.source) ? style.sources[iconset.source] : void 0;
		const sourceType = source && unbundle(source.type);
		if (!source) errors.push(new ValidationError(key, iconset.source, `source "${iconset.source}" not found`));
		else if (sourceType !== "raster-array") errors.push(new ValidationError(key, iconset.source, `iconset cannot be used with a source of type ${String(sourceType)}, it only be used with a "raster-array" source type`));
	}
	return errors;
}
var VALIDATORS = {
...
function validate(options, arrayAsExpression = false) {
	...
	if (valueSpec.expression) {
		...
		if (isExpression(deepUnbundle(value))) return validateExpression(options);
	}
	if (valueSpec.type && VALIDATORS[valueSpec.type]) {
		const errors2 = VALIDATORS[valueSpec.type](options);
		if (arrayAsExpression === true && errors2.length > 0 && Array.isArray(options.value)) return validateExpression(options);
		return errors2;
	}
	return validateObject({
		...options,
...
function validateObject(options) {
	...
	const style = options.style;
	const styleSpec = options.styleSpec;
	if (!isObject(object)) return [new ValidationError(key, object, `object expected, ${getType(object)} found`)];
	let errors = [];
	for (const objectKey in object) {
		...
		const elementSpec = hasSpec ? elementSpecs[elementSpecKey] : hasWildcardSpec ? elementSpecs["*"] : void 0;
		let validateElement;
		if (Object.hasOwn(elementValidators, elementSpecKey)) validateElement = elementValidators[elementSpecKey];
		else if (hasSpec) validateElement = validate;
		else if (Object.hasOwn(elementValidators, "*")) validateElement = elementValidators["*"];
		else if (hasWildcardSpec) validateElement = validate;
		if (!validateElement) {
	...
	for (const elementSpecKey in elementSpecs) {
		...
		if (Object.hasOwn(elementValidators, elementSpecKey)) continue;
		const elementSpec = elementSpecs[elementSpecKey];
		if (elementSpec.required && elementSpec["default"] === void 0 && (!Object.hasOwn(object, elementSpecKey) || object[elementSpecKey] === void 0)) errors.push(new ValidationError(key, object, `missing required property "${elementSpecKey}"`));
	}
	return errors;
}
function validateGlyphsUrl({ key, value }) {
	...
	const str = value;
	if (!str.includes("{fontstack}")) errors.push(new ValidationError(key, value, "\"glyphs\" url must include a \"{fontstack}\" token"));
	if (!str.includes("{range}")) errors.push(new ValidationError(key, value, "\"glyphs\" url must include a \"{range}\" token"));
	return errors;
}
...
function validateStyle$1(style, styleSpec = v8) {
	return sortErrors(validateStyle$2(style, styleSpec));
}
function sortErrors(errors) {
	return errors.slice().sort((a, b) => a.line && b.line ? a.line - b.line : 0);
}
function readStyle(style) {
	if (style instanceof String || typeof style === "string" || ArrayBuffer.isView(style)) try {
	...
	while (pos < len) {
		...
		if (c === 123) tokens.push(LBRACE, start, ++pos);
			...
			while (pos < len) {
				...
				if (ch === 92) pos += s.charCodeAt(pos + 1) === 117 ? 6 : 2;
				else if (ch === 34) break;
				else pos++;
			}
			tokens.push(STRING, start, ++pos);
		} else if (c === 116) tokens.push(TRUE, start, pos += 4);
		else if (c === 102) tokens.push(FALSE, start, pos += 5);
		else if (c === 110) tokens.push(NULL, start, pos += 4);
		else {
			while (pos < len) {
				const ch = s.charCodeAt(pos);
...
function parseTokens(s, tokens) {
	let i = 0;
	const lineOffsets = [0];
	for (let j = 0; j < s.length; j++) if (s.charCodeAt(j) === 10) lineOffsets.push(j + 1);
	...
	function parseValue() {
		...
		if (type === NUMBER) return setLine(/* @__PURE__ */ new Number(+s.slice(start, end)), line);
		if (type === TRUE) return setLine(/* @__PURE__ */ new Boolean(true), line);
		if (type === FALSE) return setLine(/* @__PURE__ */ new Boolean(false), line);
		return null;
	}
	return parseValue();
}
...
//#endregion
export { validateStyle as n, expression as t };

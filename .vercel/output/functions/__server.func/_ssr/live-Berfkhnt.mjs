...
import { A as inAoi, E as createSsrRpc, F as nearest, M as isUsefulOsm, R as pullNews, b as SITES, d as FLIGHTS, f as GDELT_ARCHIVE, g as OSM_SEED, l as FEED_CHANNELS, r as AOI, t as AIRFIELDS, u as FEED_SEED, w as allVessels, x as THERMAL } from "./control-DOKcFDnK.mjs";
//#region D:/Codex-work/ahsr-20260920/node_modules/.nitro/vite/services/ssr/assets/live-Berfkhnt.js
...
async function fetchText(url, ms = 12e3) {
	...
	try {
		...
		if (res.status === 429) {
			...
			const seconds = /^\d+$/.test(header) ? Number(header) : (Date.parse(header) - Date.now()) / 1e3;
			retryAfter.set(origin, Date.now() + Math.min(3600, Math.max(60, Number.isFinite(seconds) ? seconds : 60)) * 1e3);
		}
		if (!res.ok) throw new Error(`HTTP ${res.status}`);
		return await res.text();
	} finally {
...
function pullFlights() {
	return cached("flight-observations", 6e4, pullFlightsUncached);
}
async function pullFlightsUncached() {
	const points = [
		[
			15.6,
			32.5,
